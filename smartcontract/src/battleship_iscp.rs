...
pub fn create_game(ctx: &ScFuncContext) {
    ...
    let req: Result<CreateGameRequest, serde_json::Error> = params::get_struct::<ScFuncContext, CreateGameRequest>(PARAM_CREATE_GAME_REQUEST, ctx);
    if !req.is_ok() {
    ...
    ctx.state().get_string(ACTIVE_GAME_STATE).set_value(&game_str);
}
...
pub fn join_game(ctx: &ScFuncContext) {
    ...
    let req: Result<JoinGameRequest, serde_json::Error> = params::get_struct::<ScFuncContext, JoinGameRequest>(PARAM_JOIN_GAME_REQUEST, ctx);
    if !req.is_ok() {
        ctx.log("Request could not be deserialized");
        return
    }    
    ...
    ctx.log(&format!("Game {} was joined by player {}", game_ref.id, req_body.player_name)[..]);
...
pub fn init_field(ctx: &ScFuncContext) {
    ...
    let req: Result<InitFieldRequest, serde_json::Error> = params::get_struct::<ScFuncContext, InitFieldRequest>(PARAM_INIT_FIELD_REQUEST, ctx);
    if !req.is_ok() {
        ctx.log("Request could not be deserialized");
        return
    }    
...
pub fn make_move(ctx: &ScFuncContext) {
    ...
    let req: Result<MoveRequest, serde_json::Error> = params::get_struct::<ScFuncContext, MoveRequest>(PARAM_MOVE_REQUEST, ctx);
    if !req.is_ok() {
        ctx.log("Request could not be deserialized");
        return
    }    
    ...
    if game.mediator.player_a.as_ref().unwrap().own_field.sunked_ships == utils::ALL_SHIPS {     
        let winner_id = game.mediator.player_a.as_ref().unwrap().id.to_string();   
        game.won(&winner_id);      
    }
    if game.mediator.player_b.as_ref().unwrap().own_field.sunked_ships == utils::ALL_SHIPS {
...
pub fn quit_game(ctx: &ScFuncContext) {
    ...
    let req: Result<QuitGameRequest, serde_json::Error> = params::get_struct::<ScFuncContext, QuitGameRequest>(PARAM_QUIT_GAME_REQUEST, ctx);
    if !req.is_ok() {
        ctx.log("Request could not be deserialized");
        return
    }    
    ...
    // Settle the game
    ctx.log(&format!("Player {} wants to forfait game {}.", caller_agent_id, game.id)[..]);
    if game.mediator.player_a.as_ref().unwrap().id == caller_agent_id {        
        let winner_id = game.mediator.player_b.as_ref().unwrap().id.to_string();   
        game.won(&winner_id);
    }
...
pub fn contract_creator_only_function(ctx: &ScFuncContext) {
...
pub fn chain_owner_only_function(ctx: &ScFuncContext){
...
pub fn get_game(ctx: &ScViewContext) {
    ...
    // Reads argument called "quitGameRequestKey" and tries to deserialize it to the QuitGameRequest structure
    let req: Result<GetGameRequest, serde_json::Error> = params::get_struct::<ScViewContext, GetGameRequest>(PARAM_QUIT_GAME_REQUEST, ctx);
    if !req.is_ok() {
        ctx.log("Request could not be deserialized");
        return
    }    
    ...
    if req_body.game_id != game.id {
        ctx.log(&format!("Game {} not found.", req_body.game_id)[..]);
        ctx.results().get_string(PARAM_GAME_STATE_RESPONSE).set_value("");
        return
    }

    // Attach the game state to the responce with the key "gameStateResponseKey"
    ctx.results().get_string(PARAM_GAME_STATE_RESPONSE).set_value(&game_str);
}
