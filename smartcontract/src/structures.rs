...
impl Game {
    ...
    pub fn is_ready(&self) -> bool {
        ...
        && self.mediator.player_a.as_ref().unwrap().own_field.field != [[structures::Status::Empty; 12]; 12] 
        && self.mediator.player_b.as_ref().unwrap().own_field.field != [[structures::Status::Empty; 12]; 12]
    }

    pub fn is_player(&self, caller_agent_id: String) -> bool {
        (self.mediator.player_a.is_some() && self.mediator.player_a.as_ref().unwrap().id == caller_agent_id) 
        || (self.mediator.player_b.is_some() && self.mediator.player_b.as_ref().unwrap().id == caller_agent_id)
    }
