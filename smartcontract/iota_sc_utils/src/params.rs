...
/// Tries to get &str parameter to deserialize it to the given structure type. Returns an error if it can't find and deserialize it.
pub fn get_struct<TContext : ScBaseContext, T: serde::de::DeserializeOwned>(param_name : &str, ctx: &TContext) -> Result<T, serde_json::Error> {
    let json_str = get_string(param_name, ctx);
